package com.kevin.particletest;
...
public class ParticleTest implements ApplicationListener {
			...
			public boolean keyDown(int keycode) {
				...
				if (keycode == Input.Keys.DPAD_UP)
					...
					if (emitter.isComplete())
						emitter.reset();
					particleCount = (int) (emitter.getEmission().getHighMax()
							* emitter.getLife().getHighMax() / 1000f);
	...
	public void render() {
		...
		if (fpsCounter > 3) {
			...
			Gdx.app.log("libgdx", activeCount + "/" + particleCount
					+ " particles, FPS: " + Gdx.graphics.getFramesPerSecond());
		}
	}
